...
sed -E -e 's#<script src="config.js[^"]*"></script>#<script>window.MISCUSI_CONFIG={}</script>#' -e 's#(<script src="app.js[^"]*"></script>)#<script src="_test.js"></script>#' "$SITE/index.html" > "$SITE/_test.html"
