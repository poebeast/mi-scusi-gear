...
full = 'full.png'
shot(full, union(R['top'], R['charbar']), [(1, R['charsel'], True), (2, R['nick'], True), (3, R['lvl'], True), (4, R['save'], True)], '01-character')
...
shot(full, [R['dmg'][0], R['dmg'][1], R['dmg'][2], R['tedit'][1] + R['tedit'][3] - R['dmg'][1]], [(1, R['att'], True), (2, R['tsel'], 'top'), (3, R['swap'], True), (4, R['dctl'], True), (5, R['tline'], True), (6, R['tgear'], True), (7, R['tctl'], True)], '09-damage-setup')
hdr = [(n, (R[k][0] + R[k][2] - 30, R[k][1] + 4, 24, 20), 'top') for n, k in [(1, 'thHit'), (2, 'thCyc'), (3, 'thDps'), (4, 'thKill')]]
shot(full, [R['table'][0], R['table'][1] - 20, R['table'][2], R['table'][3] + 20], hdr + [(5, R['rot'], True), (6, R['rowOff'], True)], '10-damage-table')
shot('tbuffs.png', RB['dlg'][:3] + [560], [], '11-target-buffs', pad=0)
shot(full, [R['buffs'][0], R['buffs'][1], R['buffs'][2], R['bg0'][1] + R['bg0'][3] - R['buffs'][1]], [(1, R['bg0'], True), (2, R['buff0'], 'left'), (3, R['buffOn'], 'top'), (4, R['tgBadge'], 'left'), (5, R['removeAll'], True)], '12-buffs')
